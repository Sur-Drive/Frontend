...
//     if (!isValid || resetPassword.isPending) return;
...
//     try {
//       await resetPassword.mutateAsync({ password, confirmPassword });
//       onComplete();
//     } catch (err: any) {
...
//   const errorMessage =
//     localError || (resetPassword.error as Error)?.message || "";
...
//             className={`mt-4 h-14 rounded-2xl font-semibold text-lg text-white transition-all flex items-center justify-center mb-14 ${
//               isValid && !resetPassword.isPending
//                 ? "bg-[#6E43A3]"
//                 : "bg-purple-300 cursor-not-allowed"
//             }`}
//             whileTap={
//               isValid && !resetPassword.isPending ? { scale: 0.97 } : {}
//             }
//             initial={{ opacity: 0, y: 30 }}
//             animate={{ opacity: 1, y: 0 }}
//             transition={{
...
export default function CreateNewPassword({
  ...
  const handleSubmit = async () => {
    ...
    try {
      await resetPassword.mutateAsync({ password, confirmPassword, sessionId });
      onComplete();
    } catch (err: any) {
      console.error("Reset password failed:", err);
    }
