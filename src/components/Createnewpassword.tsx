...
export default function CreateNewPassword({ onComplete }: CreateNewPasswordProps) {
  ...
  const errorMessage = localError || (resetPassword.error as Error)?.message || ''
        ...
        >
            ...
            >
              ...
              {password && (
                <p className={`mt-1 text-sm font-medium ${
                  ...
                  strength.color === 'bg-amber-500' ? 'text-amber-600' : 'text-emerald-600'
                }`}>
                  {strength.label}
                </p>
              )}
          ...
          <motion.button
            ...
            className={`mt-4 h-14 rounded-2xl font-semibold text-lg text-white transition-all flex items-center justify-center ${
              isValid && !resetPassword.isPending ? 'bg-[#6E43A3]' : 'bg-purple-300 cursor-not-allowed'
            }`}
            whileTap={isValid && !resetPassword.isPending ? { scale: 0.97 } : {}}
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
