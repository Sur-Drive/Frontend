...
} from "./map/AddressAutocompleteInput";
...
export default function AddReportModal({
  ...
  const isSubmitting =
    createHazard.isPending || createHazardWithPhoto.isPending;
  ...
  const handleSubmit = async () => {
    ...
    if (!finalCoords) {
      ...
      try {
        ...
        setLocationError(
          "Could not find that address — try picking a suggestion from the dropdown.",
        );
        return;
      } finally {
        ...
        {step === "form" ? (
          <>
            ...
            <div className="flex-1 min-h-0 px-5 pb-5 overflow-y-auto sm:px-6 sm:pb-6">
              {/* Hazard type */}
              <p className="mb-2 text-sm sm:text-[15px] font-medium text-gray-500">
                Hazard type
              </p>
              ...
              {/* Severity */}
              <p className="mb-2 mt-4 text-sm sm:text-[15px] font-medium text-gray-500 sm:mt-6">
                Severity
              </p>
              ...
              {photos.length > 0 && (
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {photos.map((p, i) => (
                    <div
                      key={p.previewUrl}
                      className="relative overflow-hidden rounded-lg aspect-square sm:rounded-xl"
                    >
                      <img
                        src={p.previewUrl}
                        alt=""
              ...
              {!coords && !locationAddress.trim() && (
                <p className="mt-1.5 ml-1 text-[11px] sm:text-xs text-gray-400">
                  Search an address, pick a suggestion, or tap "Use my
                  location".
                </p>
              )}
              {locationError && (
          ...
          <div className="flex flex-col items-center justify-between h-full px-6 py-10 text-center sm:px-8 bg-emerald-700 sm:py-14">
            <div className="flex flex-col items-center justify-center flex-1">
              <div className="flex items-center justify-center w-20 h-20 rounded-full shadow-lg sm:h-28 sm:w-28 bg-emerald-500">
                <Check
                  size={40}
                  className="text-white sm:w-14 sm:h-14"
                  strokeWidth={3}
                />
              </div>
              <h2 className="mt-5 text-2xl font-extrabold text-white sm:mt-8 sm:text-3xl">
                Report submitted
              </h2>
              <p className="mt-2 sm:mt-3 max-w-xs text-sm sm:text-[15px] leading-relaxed text-emerald-100">
                Thanks for keeping the road safer. Your report is now visible to
                nearby drivers.
              </p>
            </div>
