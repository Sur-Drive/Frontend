...
export const navArrowPinHtml = `
  <div style="position:relative;width:36px;height:36px;">
    ...
    <div style="position:absolute;inset:4px;border-radius:50%;background:#0ea5e9;border:3px solid white;display:flex;align-items:center;justify-content:center;">
...
// Bumped from the original 28px/14px so the driver's position is easy to
// spot at a glance while moving — matches navHeadingArrowHtml's size below.
export const NAV_ARROW_ANCHOR: [number, number] = [18, 18]
...
export function destinationPinHtml(color = '#ea4335'): string {
  return `
    <div style="position:relative;width:40px;height:54px;filter:drop-shadow(0 3px 6px rgba(0,0,0,0.35));">
      <svg width="40" height="54" viewBox="0 0 34 46" xmlns="http://www.w3.org/2000/svg">
...
export const DESTINATION_PIN_ANCHOR: [number, number] = [20, 54]
...
export const START_PIN_ANCHOR: [number, number] = [11, 11]
...
// the marker travels along the path). Draws a directional arrow for
// walking/cycling/motorcycle, or a top-down car for driving — matching
// how Google Maps swaps its live-position glyph per travel mode.
export function navHeadingArrowHtml(
  headingDeg: number,
  color = '#0ea5e9',
  mode: 'driving' | 'walking' | 'cycling' | 'motorcycle' = 'walking',
): string {
  const glyph =
    mode === 'driving'
      ? `
        <svg viewBox="0 0 24 24" width="20" height="20" fill="white">
          ...
          <rect x="8.3" y="5.8" width="7.4" height="4" rx="1" fill="${color}" />
          <rect x="8.3" y="13.2" width="7.4" height="4" rx="1" fill="${color}" />
        </svg>`
      : `
        <svg viewBox="0 0 24 24" width="18" height="18" fill="white">
          <path d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71z"/>
