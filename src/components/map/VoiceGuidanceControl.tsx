...
// //             className="relative flex items-center justify-center flex-shrink-0 w-8 text-white h-9 rounded-xl"
// //           >
// //             <Settings2 size={16} />
// //             {hasAfricanVoice && (
// //               <span
...
//           onClick={toggleMuted}
//           aria-label={muted ? "Unmute voice guidance" : "Mute voice guidance"}
//           className="flex items-center justify-center flex-shrink-0 w-6 h-6 text-white rounded-lg sm:w-10 sm:h-10 sm:rounded-xl bg-white/20"
//         >
//           {muted ? (
//             <VolumeX size={14} className="sm:hidden" />
//           ) : (
//             <Volume2 size={14} className="sm:hidden" />
//           )}
//           {muted ? (
...
//           aria-label={sliderOpen ? "Hide volume slider" : "Show volume slider"}
//           aria-expanded={sliderOpen}
//           className="flex items-center justify-center flex-shrink-0 w-3 h-6 sm:w-4 sm:h-9 text-white/70"
//         >
//           {sliderOpen ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
//         </button>
//         {showSettings && (
//           <button
//             onClick={() => setSettingsOpen((v) => !v)}
//             aria-label={settingsOpen ? "Hide voice settings" : "Voice settings"}
//             aria-expanded={settingsOpen}
//             className="relative flex items-center justify-center flex-shrink-0 w-5 h-6 text-white rounded-lg sm:w-8 sm:h-9 sm:rounded-xl"
//           >
//             <Settings2 size={13} className="sm:hidden" />
//             <Settings2 size={16} className="hidden sm:block" />
//             {hasAfricanVoice && (
//               <span
//                 className="absolute top-0.5 right-0 w-1.5 h-1.5 rounded-full bg-emerald-400"
//                 title="An African-language voice is available"
//               />
//             )}
//           </button>
...
export default function VoiceGuidanceControl({
  ...
  return (
    <div className={`relative flex flex-col items-center ${className}`}>
      ...
      {showSettings && settingsOpen && (
        <div className="absolute right-0 z-10 w-72 px-3 py-3 space-y-3 bg-white shadow-lg top-full mt-1.5 rounded-xl">
          <div>
            <label className="flex items-center gap-1 mb-1 text-[10px] font-semibold tracking-wide text-gray-500 uppercase">
              <Globe2 size={11} /> Language
            </label>
            <select
              value={language ?? ""}
              onChange={(e) => setLanguage!(e.target.value || null)}
              className="w-full px-2 py-1.5 text-xs bg-gray-50 border border-gray-200 rounded-lg text-gray-800 whitespace-normal"
            >
              <option value="">Auto (device default)</option>
              {languages!.map((l) => (
                <option key={l.lang} value={l.lang} title={l.label}>
                  {l.isAfrican ? "🌍 " : ""}
                  {l.label}
                </option>
              ))}
