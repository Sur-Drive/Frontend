...
function describeUpcomingHazard(
  hazards: unknown[] | undefined,
...
export default function RouteResultsPage() {
  ...
  const simulationDurationMs = useMemo(
    () =>
      Math.max(
        8000,
        Math.min(40000, (activeRoute?.durationInSeconds ?? 60) * 40),
      ),
    [activeRoute],
  );
  ...
  const upcomingHazard = useMemo(
    () => describeUpcomingHazard(activeRoute?.hazards),
    [activeRoute],
  );
  ...
  const remainingKm = activeRoute
    ? Math.max(0, activeRoute.distance * (1 - progress))
    : 0;
  const etaMin = activeRoute
    ? Math.max(0, Math.round(activeRoute.duration * (1 - progress)))
    : 0;
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-[#e4e4e4]">
      ...
      <div className="absolute left-0 right-0 z-20 pl-4 pr-24 lg:pr-4 mx-auto max-w-[430px] lg:max-w-md bottom-[calc(env(safe-area-inset-bottom)+1rem)]">
        <div className="overflow-hidden bg-white shadow-lg rounded-2xl">
          ...
          >
            ...
            <svg
              viewBox="0 0 24 24"
              className={`w-4 h-4 text-gray-400 transition-transform ${tripSheetExpanded ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              strokeWidth="2.4"
              strokeLinecap="round"
              ...
              >
                <div className="px-3 pb-3">
                  <button
                    ...
                    className="w-full h-10 text-sm font-semibold text-white bg-red-500 rounded-full active:scale-[0.98] transition"
                  >
                    End Trip
                  </button>
                </div>
...
function MiniStat({
  ...
  return (
    <div>
      ...
      <p
        className={`text-sm font-extrabold ${accent ? "text-amber-500" : "text-gray-900"}`}
      >
        {value}
      </p>
    </div>
  ...
  return (
    <div className="absolute inset-0 z-40 flex flex-col bg-red-500">
      ...
      <div className="px-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] space-y-3">
        <button className="w-full font-bold text-red-600 bg-white rounded-full h-14">
          Cancel SOS
        </button>
...
//                   <button
//                     onClick={handleEndTrip}
//                     className="w-full h-11 font-semibold text-white bg-red-500 rounded-full active:scale-[0.98] transition"
//                   >
//                     End Trip
//                   </button>
//                 </div>
