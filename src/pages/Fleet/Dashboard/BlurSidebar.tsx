...
const BlurSidebar: React.FC<BlurSidebarProps> = ({
    children,
    status = "pending",
}) => {
    ...
    const getStatusConfig = () => {
        switch (status) {
            case "pending":
                return {
                    icon: <Clock size={16} className="text-[#F2A618]" />,
                    label: "KYC Pending",
                    bgColor: "bg-[#F2A61820]",
            ...
            case "expired":
                return {
                    icon: <AlertCircle size={16} className="text-[#FE3F21]" />,
                    label: "Subscription Expired",
                    bgColor: "bg-[#FE3F2124]",
                    textColor: "text-[#FE3F21]",
                };
            case "payment_pending":
                return {
                    icon: <CreditCard size={16} className="text-[#F2A618]" />,
        ...
        <div className="flex h-screen">
            ...
            >
                <div className="flex flex-col h-full">
                    ...
                    {/* Status Badge */}
                    <div
                        className={`mb-6 px-4 py-2 rounded-lg ${statusConfig.bgColor} flex items-center gap-2`}
                    >
                        {statusConfig.icon}
                        {!collapsed && (
                            <span
            ...
            {/* Main Content */}
            <div className="flex-1 overflow-auto bg-[#F3F0F7]">
                {/* Status Banner at top of content */}
                <div
                    className={`${statusConfig.bgColor} border-b border-[#E5E7EB] px-6 py-3 flex items-center justify-between`}
                >
                    ...
                    <button
                        onClick={() => {
                            if (status === "pending") {
                                navigate("/fleet/kyc-pending");
                            } else if (status === "expired") {
                                navigate("/fleet/subscription/renew");
                            } else {
                                navigate("/fleet/payment/pending");
                            }
                        }}
                        className="px-4 py-1.5 rounded-lg bg-[#6E43A3] text-white text-sm hover:opacity-90 transition font-medium"
                        style={{ fontFamily: "Outfit" }}
                    >
                        Resolve Now
                    </button>
