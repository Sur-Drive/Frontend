...
export async function getNearbyHazards(
  ...
  if (!res.ok) {
    ...
    throw new ApiError(
      `Failed to fetch hazards: ${res.status} ${err}`,
      res.status,
    );
  }
  ...
  console.log(
    ...
    Array.isArray(data)
      ? data.map((h: any) => h?.confirmations?.userConfirmation)
      : data,
  );
...
export async function confirmHazard(
  ...
  const data = await res.json();
  console.log(
    "[confirmHazard] response hazard.confirmations:",
    data?.hazard?.confirmations,
  );
